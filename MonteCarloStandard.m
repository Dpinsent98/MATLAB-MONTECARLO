...
%1.European Put Call, 2.Down In, 3. Down Out, 4. Up In, 5. Up Out, 6.
%Double Out, 7. Double In
OptionType =5
...
%Compute Path
N(i,j) = randn(1);
S1(i,j) = S1(i,j-1)*exp((r - 0.5*V^2)*(TI) + V * sqrt(TI) * N(i,j));
...
CPayOff(i) = CPayOff1(i); 
PPayOff(i) = PPayOff1(i); 
end
%Option Price
C = mean(CPayOff)*exp(-r*T)
P = mean(PPayOff)*exp(-r*T)
